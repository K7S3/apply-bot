...
| `debrief` | Post-interview debriefs, voice or typed: a guided 6-prompt session (adaptive follow-ups, local vosk/whisper.cpp STT with typed fallback, offline heuristic + optional local-LLM extraction into questions/answers/weak spots/action items). `debrief due [--days N]` lists recent interviews missing a debrief (also surfaced as nudges in the dashboard); past weak spots feed into prep packs built with `--app-id`; `debrief export <id> --format md` exports transcript + summary + weak spots as Markdown. See `docs/debrief_voice.md`. |
