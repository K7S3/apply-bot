...
| `pm` | Product-manager interview track: `questions` (company-reported PM question bank), `concepts` (metrics/experimentation deep-dives), `drills` (timed practice), `mock` (AI-interviewer loops), `stories` (STAR bank), `scores` (practice trends), `prep` (company questions + metrics to know + teardown assignment + day-before checklist, saved as Markdown), `teardown` (blank product-teardown templates with RICE scoring tables). See [docs/pm_track.md](docs/pm_track.md). |
