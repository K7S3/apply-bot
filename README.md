...
| `dashboard` | Local web UI (127.0.0.1 only): funnel visualization, sortable/filterable applications table, curated-jobs workflow (curate from the UI, dismiss, tailor shortcut), match/tailor lab with keyword-coverage chips, prep cards, salary widget, an **Import your data** section (export guides, drag-and-drop upload, proposal confirm/reject), and a **Privacy** section (data inventory, PII scan, export/purge/audit links). |
| `privacy` | Privacy dashboard: `inventory` (everything candid holds, sizes + record counts), `show` (per-record viewer), `scan` (PII scan with masked matches), `egress` (static "what leaves this machine" report), `export` (archive a category, `--redact` strips PII), `purge` (per-category delete, `--export-first`, `--keep-days`), `retention` (per-category retention policies + `run --dry-run`), `nuke` (full wipe, type DELETE to confirm, exports preserved), `audit` (append-only action log). Destructive actions confirm before acting. |
