...
| `whiteboard` | Whiteboard practice mode: 10 seeded system-design drills with a timed phase plan, describe-your-diagram narration flow, expected-component checklist with coverage scoring, reference outlines for self-comparison, structured rubric feedback (clarity/completeness/depth/communication), follow-up question bank with quiz mode, and session history with per-drill progress trends. See `docs/whiteboard.md`. |
