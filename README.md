...
| `brief` | Interviewer briefs: a roster of who's interviewing you (role, round, background *you* looked up - candid never scrapes), per-role likely question angles with tells, topic-to-deep-dive mapping from their public talks, a no-repeat panel strategy, smart questions to ask each interviewer, per-interviewer debriefs, and a one-command Markdown brief. Folds into `prep` packs automatically when interviewers are known. |
