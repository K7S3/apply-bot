...
| `context` | Multiple job-search profiles (batch-73): `list`, `create`, `use`, `current`, `rename`, `delete`, `show`, `export`, `import`. Each profile gets its own tracker, onboarding profile, prep packs, and tailored outputs. Your existing data is the `default` profile — nothing moves, zero migration. See [docs/profiles.md](docs/profiles.md). |
| `--profile NAME` | Global flag (goes before the subcommand): run any command against one profile, e.g. `python -m candid --profile mle track list`. Resolution order: `--profile` > `CANDID_PROFILE` env var > current profile (`context use`) > `default`. The dashboard reads `?profile=` the same way; `?profile=all` shows every profile combined. |
