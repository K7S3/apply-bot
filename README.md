...
| `practice` | Live-coding practice harness: timed sessions with milestone warnings, distraction-free **focus mode** (hints/solutions locked while the clock runs), think-aloud prompts, plan/code/test/review phase budgets, judge-backed attempts, per-session review, and automatic pattern tags (`topic:`, `diff:`, plus behavioral tags like `first-try-solve`, `hint-dependent`, `time-pressure`). Session history, solve-rate stats with streaks, a spaced-repetition revisit queue, and named routines (`warmup`, `interview-sim`, `dp-drill`). See [docs/practice.md](docs/practice.md). |
...
- [docs/practice.md](docs/practice.md) — the live-coding practice harness (timed sessions, reviews, tags, queue, routines)
