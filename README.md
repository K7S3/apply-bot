...
| `archive` | Cold archival: compress stale search/curation data, archive closed application cycles, search inside archives without restoring, verify checksums, export/import portable `.candid-cold` files, storage stats, retention policies (`cycles`, `compress`, `search`, `verify`, `export`, `import`, `stats`, `prune`, `policy`, `restore-app`, `restore-data`). |
