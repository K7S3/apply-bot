...
| `track` | Application tracker: add / list / update / stats / search / export-csv, with funnel + response/interview/offer rates and per-status next-action hints. Re-adding an existing company+role returns the existing record instead of duplicating. `track list` also shows an intro column with the warm-outreach status per company. |
| `warm` | Warm-intro job ranking from your own LinkedIn export (1st-degree connections only): `rank` blends connection strength with match scores, `queue` is the prioritized outreach queue (one row per company, with a last-contact hook for Gmail history), `map` groups insiders into hiring / recruiting / engineering, `draft` writes an intro-request message, `status` records outreach (none / asked / introduced / applied), and `link` ties a company to a tracker app id. See docs/warm_intros.md. |
