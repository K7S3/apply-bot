...
| `research` | **AI research track** (all local, template-based, never invents papers/results): paper library (`papers add/list/get/search/status/notes`) with reading statuses and notes; paper **deep-dive drills** (`papers drill`) with 8 question categories and self-scored confidence stats; **publication framing** (`frame`) that turns your project notes into candidate titles + an abstract skeleton with explicit `[FILL IN]` slots (no real venue names, no fabricated claims); **research statement builder** (`statement build/check`) with length guidance and Markdown export; **research-taste prep** (`taste practice`) from a curated question bank with per-category stats and drill-next suggestions; **advisor/committee hard-Q&A** (`hardqa`) generated from *your* project text with answer checklists; **prior-work map** (`map`) with timeline, area/technique coverage, and heuristic gap observations (labeled as suggestions); **rebuttal trainer** (`rebuttal`) that classifies *your pasted* critiques and tracks addressed/pending; **talk outline builder** (`talk --minutes 15`) with time budgets and speaker-note prompts; **notes export** (`export-notes`/`export-digest`) to Obsidian-friendly Markdown. |
