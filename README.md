...
| `update` | Check for new releases: changelog preview, upgrade guidance, opt-out. See [docs/selfupdate.md](docs/selfupdate.md). |
...
- `mock ai` → Gemini, **only** for conversational interview dialogue, only
  when you run it.
- `update check` → the GitHub releases API (`api.github.com`, public, no
  auth), only when you run it or when the enabled startup nudge fires.
  See [docs/selfupdate.md](docs/selfupdate.md) for details and opt-out.
...
- [docs/selfupdate.md](docs/selfupdate.md) - the update checker: release
  checks, upgrade guidance, and opt-out
