...
| `quality` | Data-quality engine - scans the tracker + profile for duplicates, bad statuses, missing fields, and stale records - grouped report with a 0–100 score, `--json` for scripting, `--min-severity`, and `--fix` / `--dry-run` for safe auto-fixes (whitespace, status casing, missing `date_updated`). See [docs/quality.md](docs/quality.md). |
