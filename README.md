...
| `network` | Networking follow-up cadence manager: a contact book, post-event follow-up sequences (conference / meetup / informational / recruiter call / coffee chat / referral intro), touchpoint logging, relationship-warmth tracking with decay, value-add touch ideas + drafts, due follow-up reminders, double opt-in intro builder, quarterly re-engagement planner, thank-you drafts, and an activity report. See [docs/networking.md](docs/networking.md). |
