...
| `negoseq` | Negotiation email **sequence builder**: timed multi-step drafts (counter, silence nudge, call request, deadline reply, acceptance, decline) in warm / professional / assertive tones, business-day timing guidance, BATNA-aware talking points, an anchoring calculator, recruiter-pushback replies, a multi-lever tradeoff menu, a call/voicemail script, and a sequence tracker (`negoseq track`) so you always know what to send next. |
