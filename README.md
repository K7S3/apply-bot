...
| `track` | Application tracker: add / list / update / stats / search / export-csv, with funnel + response/interview/offer rates and per-status next-action hints. Re-adding an existing company+role returns the existing record instead of duplicating. `--kind postdoc` tracks academic applications with PI, lab, funding source, deadline, and start date. |
| `jobs` | Curate open postings from public feeds, score them against your profile, and save the good ones to the tracker. `--days N` for recency, `--min-score N` to gate tracker writes, cross-source dedupe, phrase-aware ranking. Opt-in academic sources via `feeds` (university/research RSS). See [coverage](#job-source-coverage-honest). |
...
| `salary` | Salary intelligence: import DOL H-1B LCA disclosure data (CSV), parse posted ranges, look up p25/median/p75 by company + title with per-row source attribution, title-level aggregation across companies, and the NIH NRSA postdoc stipend scale (`salary lookup --postdoc --years N`). |
...
| `feeds` | Manage university/research RSS job feeds: a user registry (`feeds add` / `list` / `remove`, live-verified on add) seeded with verified public feeds (Nature Careers, Inside Higher Ed, THE UniJobs, HigherEdJobs). Opt-in sources for `jobs curate`. |
| `fellowships` | Curated postdoc/doctoral fellowship database (12 verified programs with real URLs): `fellowships list` and `fellowships upcoming` (deadlines always labeled approximate); upcoming deadlines surface as nudges. |
| `labs` | Lab/PI watchlist: `labs add "Name"` and `labs check` scans curated jobs for watched labs, PIs, and fellowships with word-boundary matching (no "lab"-in-"collaborate" false positives). |
| `academic` | Academic hiring-season calendar: the typical faculty/postdoc cycle by month (`academic calendar`, `academic season-status`). Every entry labeled typical/approximate, never exact dates. |
| `academic-cv` | Academic CV export (plain text or LaTeX) from your profile: education, appointments, publications, teaching, grants, service. Sections appear only when your profile has them; nothing is invented. |
| `research-statement` | Grounded research-statement draft distilled from your profile's publications, projects, and experience, with a clearly-marked TODO scaffold for future directions. Never invents plans or results. |
| `academic-digest` | One-command academic briefing: upcoming fellowship deadlines, watched-lab hits in the latest curated jobs, and this month's hiring-season milestones. Each section degrades gracefully when its data is absent. |

## Job-source coverage (honest)

`jobs curate` pulls from **two free, no-login JSON feeds** by default: Arbeitnow and
RemoteOK. Opt-in academic sources are available too: `feeds` manages a registry
of public university/research RSS feeds (seeded with verified ones: Nature
Careers, Inside Higher Ed, THE UniJobs, HigherEdJobs faculty feeds) that
`jobs curate --sources` can include. That's the entire coverage. It does *not*
search the web at large, read company career pages, or touch anything behind a
login — and the CLI never claims otherwise. Most individual university boards
sit behind logins (Symplicity/Workday) and are out of scope. Adding a new
public feed is a ~20-line adapter; see [docs/adding_sources.md](docs/adding_sources.md).
...
- **NIH NRSA postdoc stipend scale** (`salary lookup --postdoc [--years N]`): the
  published FY2026 Ruth L. Kirschstein stipend levels by years of experience.
  This is a stipend floor, not a market pay rate.
