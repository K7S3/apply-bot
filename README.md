...
| `metrics` | Bullet metric helper: scans your profile bullets for unquantified work, prompts you for the numbers, suggests tighter phrasings built only from numbers you supply, and tracks metric coverage. Declined bullets are never auto-filled; `metrics audit` proves every number traces back to you. See [docs/metrics.md](docs/metrics.md). |
