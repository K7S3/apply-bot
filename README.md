...
| `ritual` | Pre-interview confidence routine: `logistics` / `timeline` / `materials` / `dossier` / `warmup` / `techcheck` / `calm` / `countdown` for interview day, plus `interviewers` (record the panel, see likely question angles per interviewer) and `cooldown` (post-interview guided cooldown + debrief capture). See [docs/ritual.md](docs/ritual.md). |
