...
| `guide` | Direct-apply vs portal guidance: per-posting ranked channel advice (`posting`), ATS portal notes (`ats`), referral-path detection from your LinkedIn export (`referral`), application-channel logging (`log`, `track --channel`), channel success stats (`stats`), and cold direct-apply email drafts (`email`). See [docs/direct_apply.md](docs/direct_apply.md). |
