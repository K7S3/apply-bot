...
| `jobs` | Curate open postings from public feeds, score them against your profile, and save the good ones to the tracker. `--days N` for recency, `--min-score N` to gate tracker writes, `--stage seed,series-a` and `--size 1-50` startup filters (stage/size inferred from posting text or your startup registry), cross-source dedupe, phrase-aware ranking. See [coverage](#job-source-coverage-honest) — it's two public APIs, not the whole web. |
| `startups` | Startup-ecosystem toolkit: a curated startup registry (`import` a CSV, `list`/`add`/`remove`, `set-stages` for preferred funding stages), hiring signals computed from your own curation history (`signals`: posting velocity, trend, new-vs-repeat), role-fit ranking (`rank`), a fuzzy watchlist with no-duplicate alerts (`watch`), and a digest of new matches, signal movers, and startup interviews (`digest`). |
| `prep` | Role-aware interview prep pack: real reported company questions (with source links) or an explicit "no verified questions" fallback, gap-prioritized concept deep-dives, STAR prompts built from *your* resume bullets, company-research checklist, comp talking points, day-before checklist. `--startup` appends a startup-interview-loop section (founder chat, technical deep-dive, take-home planning guidance, reference prep). Exportable Markdown. |
...
| `offer` | Normalize offers (base + bonus + sign-on + equity/vesting + benefits) into comparable $/yr, side-by-side tables, rough tax note, and markdown export (`offer export`). Startup extras: `offer equity-notes` gives a due-diligence checklist with plain-English explainers (strike price, 409A, cliff, ISO/NSO, exercise window; missing values become "ask the company" prompts, never invented numbers), and `offer compare-startup` models cash-vs-equity tradeoffs over 4 years under labeled growth assumptions (educational only, not tax advice). |
