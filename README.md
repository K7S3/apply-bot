...
| `prep` | Role-aware interview prep pack: real reported company questions (with source links) or an explicit "no verified questions" fallback, gap-prioritized concept deep-dives, STAR prompts built from *your* resume bullets, company-research checklist, comp talking points, day-before checklist. Exportable Markdown. `--track security` prepends the security-engineer track section. |
| `security` | Security-engineer interview track (see [docs/security_track.md](docs/security_track.md)): researched question bank with sources (`questions`), concept deep-dives (`concepts`), interactive STRIDE threat-modeling drills (`threat-model`), secure system-design drills (`design-drill`), scored mock interviews (`mock`), skill-gap assessment (`gaps`), STAR story prompts (`stories`), and interview-loop guides (`loop`). |
