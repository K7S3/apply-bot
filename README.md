...
| `profile github` | Link your public GitHub repos via the no-key GitHub API (keyword extraction from names/descriptions/topics/languages). Cached locally with 24h TTL; `--refresh` refetches; works offline from cache. `match` then cites the most relevant project per JD (keyword overlap only, honestly labeled) and `tailor` can reference it. |
| `match` | Score any JD 0–100 (skills / seniority / domain / title fit) with a GO / CONDITIONAL / NO-GO verdict. Section-weighted skill extraction, explicit "N+ years" handling, per-skill JD evidence, and "how to close it" pointers for missing must-haves. Skill aliases (`k8s`=Kubernetes, `js`=JavaScript, ...) normalize in both directions during scoring — aliases only add matches, never remove. Accepts text, a file, a URL, or stdin. `--json` for scripting. |
...
| `track` | Application tracker: add / list / update / stats / search / export-csv, with funnel + response/interview/offer rates and per-status next-action hints. Re-adding an existing company+role returns the existing record instead of duplicating. `track export-ics` writes interviews (detected from notes) + follow-up reminders as a single RFC 5545 `.ics` file (stdlib only) — entries with no detectable date are skipped with a note, never given a fabricated time. `track ghosts [--days N]` flags applications quiet for N days (default 21, terminal states never flagged) with a check-in-draft or archive suggestion per app. |
| `profile autofill` | Application autofill kit: form-ready, copy-pasteable work history / skills / education from your profile, plus EEO voluntary-disclosure safe defaults ("I don't wish to answer" / "Decline to self-identify") — candid never invents disability, veteran, gender, or race answers. Clearly labeled "fills, never submits". |
| `network` | Networking CRM: log coffee chats, referrals received, and referrals given (`network add`), list them (`network list`), and see thank-yous owed (`network thanks`) — each entry points at the exact `followup thank-you` draft command, or `network thanks --draft ID` prints it inline. Stored as JSON in `candid_data/`, like the tracker. |
| `jobs` | Curate open postings from public feeds, score them against your profile, and save the good ones to the tracker. `--days N` for recency, `--min-score N` to gate tracker writes, `--min-sponsor N` to keep only likely H-1B sponsors (needs imported LCA data), `rank-remote` to rank remote roles by effective pay vs cost of living at your location (estimates). Cross-source dedupe, phrase-aware ranking. See [coverage](#job-source-coverage-honest) — it's two public APIs, not the whole web. |
| `prep` | Role-aware interview prep pack: real reported company questions (with source links) or an explicit "no verified questions" fallback, a **company brief** (Wikipedia basics + SEC EDGAR 10-K highlights, every fact source-labeled, `--no-brief` to skip), gap-prioritized concept deep-dives, STAR prompts built from *your* resume bullets, company-research checklist, comp talking points, day-before checklist. Exportable Markdown. |
| `prep brief` | Print just the company brief: funding/headcount basics from the Wikipedia API and 10-K highlights (revenue, net income, assets, employees) from SEC EDGAR for public companies. Every fact carries its source; missing data prints an explicit "no verified data found" note instead of invented numbers. Responses are cached locally under `candid_data/briefs_cache/` (`--refresh` to re-fetch, offline falls back to cache). See [docs/briefs.md](docs/briefs.md). |
| `mock` | Mock interviews: 15 seeded coding problems with a **sandboxed judge** (visible + hidden tests, hints, reference solutions; infinite loops fail fast per-test), behavioral STAR practice, system-design prompts, and an optional AI interviewer. Sandboxing limits CPU/memory/files per run; note the judge is built for running *your own* practice code, not untrusted third-party code (network is not blocked at the OS namespace level). **New:** `mock voice --kind behavioral\|coding\|design` reads the question aloud via system TTS (`say`/`espeak`/`spd-say`) and captures your answer through a pluggable STT backend (offline `typed` default), scoring key-point coverage; degrades to text mode when no TTS/mic is available. See [docs/voice.md](docs/voice.md). |
| `salary` | Salary intelligence: import DOL H-1B LCA disclosure data (CSV), parse posted ranges, look up p25/median/p75 by company + title with per-row source attribution, plus title-level aggregation across companies. `sponsor` scores per-company H-1B sponsorship likelihood 0-100 from your imported LCA rows (filing volume, approval consistency, recency) — see [docs/sponsorship.md](docs/sponsorship.md). |
| `offer` | Normalize offers (base + bonus + sign-on + equity/vesting + benefits) into comparable $/yr, side-by-side tables, rough tax note, and markdown export (`offer export`). Parse an offer letter PDF straight into a record with `offer parse letter.pdf --company X --role Y` (heuristic extraction, every field shown with its source snippet, confirmation before saving; see [docs/offer_parse.md](docs/offer_parse.md)). |
...
| `gmail classify` | Classify recruiter mail from a Takeout mbox into direct outreach / agency recruiter / interview invite / unsure, with human-readable reasons per message. Interview invites become tracker *proposals* (pending) — nothing is written until you confirm. Heuristics only, fully offline. See [docs/classify.md](docs/classify.md). |
