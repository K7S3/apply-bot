...
| `tailor` | Grounded résumé + cover letter in 4 tones and 2 lengths. Reorders *your* bullets; never invents experience. Now ends with an **ATS keyword check** (covered vs missing JD keywords) and a **what-changed** summary. `--new-grad` switches thin-experience profiles to projects-first ordering (with `--include-coursework` opt-in). |
| `track` | Application tracker: add / list / update / stats / search / export-csv, with funnel + response/interview/offer rates and per-status next-action hints. Re-adding an existing company+role returns the existing record instead of duplicating. `--internship` marks internship apps; `--return-offer` records outcomes; `track conversion-stats` shows the internship -> return-offer -> accepted funnel. |
| `jobs` | Curate open postings from public feeds, score them against your profile, and save the good ones to the tracker. `--days N` for recency, `--min-score N` to gate tracker writes, cross-source dedupe, phrase-aware ranking. `--new-grad` filters out senior-signaled postings and boosts entry-level ones; `jobs newgrad-guide` prints where entry-level roles actually appear. See [coverage](#job-source-coverage-honest) — it's two public APIs, not the whole web. |
| `prep` | Role-aware interview prep pack: real reported company questions (with source links) or an explicit "no verified questions" fallback, gap-prioritized concept deep-dives, STAR prompts built from *your* resume bullets, company-research checklist, comp talking points, day-before checklist. Exportable Markdown. `--new-grad` reweights toward fundamentals (DSA/OOP/SQL/OS) with entry-level behavioral sets and lighter system-design expectations. |
...
| `followup` | Thank-you, recruiter check-in, and referral-request drafts in your voice, with subject lines, timing advice, and tone options. `referral --new-grad` writes peer-level alumni/classmate drafts (LinkedIn connection-note length); `referrals classmates --company X` prints an alumni search checklist + templates. |
...
| `campus` | Campus recruiting timelines: seeded (not scraped) windows for fall full-time, spring, and internship recruiting with per-company-type notes; `campus timeline` and `campus deadlines`. |
| `internships` | Internship application tracker with a simplified stage model (applied -> OA -> phone -> onsite -> offer), bulk CSV import, and deadline alerts. |
| `fair` | Career fair logger: record fairs, companies/people you talked to, and follow-ups owed, with ready-to-use follow-up drafts. |
| `checklist` | First-job milestone timeline anchored to your graduation date, with check-off progress and overdue nudges. |
| `convert` | Link an internship record to the full-time application it converts into (used with `--internship` / `--return-offer` in `track`). |
