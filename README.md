...
| `prep` | Role-aware interview prep pack: real reported company questions (with source links) or an explicit "no verified questions" fallback, gap-prioritized concept deep-dives, STAR prompts built from *your* resume bullets, company-research checklist, comp talking points, day-before checklist. Exportable Markdown. `prep ds <role>` adds a Data Scientist variant: statistics refresher pointers, ML-case and SQL drill pointers, and DS-specific STAR/comp sections. |
| `ds-stats` / `ds-metrics` | DS interview track: 25 statistics/probability concept cards + quiz mode (`ds-stats review|quiz`), and a stdlib metrics toolkit computing and explaining accuracy/precision/recall/F1/AUC/log-loss/RMSE/MAE/uplift/lift/gain/NDCG/MAP/calibration (`ds-metrics <metric>`, `ds-metrics explain <metric>`). |
| `ds-case` / `ds-sysdesign` | DS interview track: 8 ML case-interview drills (churn, recommender, fraud, ETA, ad ranking, LTV, demand forecasting, moderation) and 6 ML system-design drills (feature store, retraining pipeline, shadow deploy, experimentation platform, data quality, vector search), each with interactive drill mode, rubric-based feedback, and saved session reports. |
| `ds-sql` / `ds-exp` | DS interview track: 15 SQL practice questions with a real in-memory SQLite judge (`ds-sql list|show|solve`), and A/B-test experiment-design drills plus a stdlib power/sample-size calculator (`ds-exp list|show|drill|calc`). |
| `ds-takehome` / `ds-portfolio` | DS interview track: timed take-home assignment simulator with reproducible synthetic CSVs, deadlines, and a completeness check (`ds-takehome list|show|start|submit|status|csv`), and a portfolio readiness checker + DS portfolio playbook (`ds-portfolio check|guide`). |
| `salary ds-bands` | Salary intelligence for DS titles: `data-scientist`, `senior-data-scientist`, `ml-engineer`, `data-analyst`, `ds-manager`, `research-scientist` title groups on top of the same LCA wage sources. |
