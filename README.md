...
| `jobs` | Curate open postings from public feeds, score them against your profile, and save the good ones to the tracker. `--days N` for recency, `--min-score N` to gate tracker writes, cross-source dedupe, phrase-aware ranking. Also flags **hidden gems** (high fit, low-competition signals): `jobs gems`, `jobs why-gem`, `gems employers`. See [docs/gems.md](docs/gems.md). Coverage: [two public APIs](#job-source-coverage-honest), not the whole web. |
...
| `dashboard` | Local web UI (127.0.0.1 only): funnel visualization, sortable/filterable applications table, curated-jobs workflow (curate from the UI, dismiss, tailor shortcut), **Hidden gems** panel (top gem picks from your curated state), match/tailor lab with keyword-coverage chips, prep cards, salary widget, and an **Import your data** section (export guides, drag-and-drop upload, proposal confirm/reject). |
