...
| `dashboard` | Local web UI (127.0.0.1 only): funnel visualization, sortable/filterable applications table, curated-jobs workflow (curate from the UI, dismiss, tailor shortcut), match/tailor lab with keyword-coverage chips, prep cards, salary widget, and an **Import your data** section (export guides, drag-and-drop upload, proposal confirm/reject). Accessibility: WCAG AA contrast pass over the full palette (4.5:1 text, 3:1 UI boundaries, ratios documented in the CSS and re-checked by tests); visible 3px keyboard focus indicators on all interactive elements; reduced-motion support honoring the OS setting plus a manual toggle hook (`html[data-motion="reduced"]`, contract documented in the CSS for the header checkbox). |
| `dashboard` a11y: landmarks + headings | "Skip to main content" link as the first element, banner/main/contentinfo landmarks, exactly one h1, and a strict heading hierarchy (sub-headings demoted to h3 so every section has one h2). |
| `dashboard` a11y: labelled sections | Every dashboard `<section>` points `aria-labelledby` at its own heading id, so each region is announced by name. |
| `a11y` | Audit the dashboard UI for accessibility (WCAG 2.2 AA): palette contrast ratios, skip link + landmarks + heading order, form labels, button text, image alt, `:focus-visible` and reduced-motion CSS, polite live region, table captions. Human-readable report or `--json`; exit code 1 on any failure. See [docs/accessibility.md](docs/accessibility.md). |
| `dashboard` a11y: keyboard operability | `/` focuses the applications search box, `?` opens a shortcut-help dialog, and `Esc` closes any open dialog. Sortable applications-table headers are real `<button>` elements inside `<th scope="col">` with `aria-sort` updated on every sort. The destructive dismiss-job action confirms through a real `<dialog>` (no more `confirm()`) with a focus trap, Escape-to-close, and return-focus. |
| `dashboard` a11y: live regions | A single visually-hidden `role="status"` / `aria-live="polite"` region (right after `<main>` opens) receives all async status updates through an `announce(msg)` helper: curate, match, tailor, salary lookup, upload progress, table refresh counts, proposal confirm/reject, and dismiss results. Real errors go to a second region with `role="alert"` / `aria-live="assertive"`, via `announceErr()`. |
| `dashboard` a11y: color-independent status | Status is never conveyed by color alone. Status pills pair the text label with a distinct shape glyph and an `aria-label` ("Application status: …"); score badges show the numeric score plus an `aria-label` naming the tier (high/medium/low); keyword chips pair the ✓/✗ glyph with an `aria-label` naming covered vs missing. |
| `dashboard` a11y: tables, charts, forms | The applications table has a (visually-hidden) `<caption>`, `scope="col"` on every header, and `aria-sort` on the sortable columns. The pipeline funnel chart ships a visually-hidden data table with per-stage counts (`scope="row"`) so screen readers get the same data. Every input/select/textarea has an associated `<label>` (for/id or wrapping, including the per-row status selects via `aria-label`); the required role field uses `aria-required` plus visible "(required)" text; error outputs use `role="alert"`. |
...
- [docs/accessibility.md](docs/accessibility.md) - accessibility statement: what `candid a11y` audits, WCAG 2.2 AA target, known limitations
