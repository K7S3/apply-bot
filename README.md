...
| `draft` | Context-aware email drafter: builds drafts from your tracker state and thread history (`context`, `thread`, `generate` with 7 kinds, `ladder` tone ladder, `followups` stale-app suggestions, `check` hygiene, `send-time`, `revise`, `save`/`list`/`show`/`diff` per-application draft versions, `voice-learn`). Drafts only — nothing is ever sent automatically. |
