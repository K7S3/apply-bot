...
| `federal` | Federal hiring tools: `search` (USAJOBS — needs the free key at developer.usajobs.gov, or `--samples` for fictional demos), `translate` (title+experience to GS grade band), `series` (profile skills to occupation series), `eligibility` (checklist from facts you provide), `pay` (2026 GS + locality tables), `resume-notes` (federal-resume checklist), `score` (profile vs announcement), `announce` (parse a raw USAJOBS item). `usajobs` is also an opt-in `jobs curate --sources usajobs` source. See [docs/federal.md](docs/federal.md). |
