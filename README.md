...
| `negotiate` | BATNA playbook + pre-call checklist, scenario scripts (lowball / competing offer / exploding deadline / level pushback / leveling-up / remote flexibility), salary-expectation answer scripts (range / deflect / anchor; researched from your local salary data, honest fallback when there's no data), counteroffer email drafts, and an interactive `practice` simulator that role-plays the recruiter and ends with a debriefed transcript. |
...
| `outreach` | Communication and outreach: cold outreach 4-touch sequences (`sequence`, days 0/4/11/21) and follow-up escalation ladders (`ladder`) with timing notes; LinkedIn connection notes (`connect`, or `connect-company` to draft one per top referral candidate); recruiter reply drafts (`reply`: interested / not_interested / need_details) and recruiter call prep (`call-prep`); offer announcement drafts (`announce`, LinkedIn post + network message); resignation letters and talking points (`resign --talking-points`); and a tone tuner (`tone`: retune a draft to warm/formal/concise/enthusiastic/direct, detect a draft's tone, or match your voice from a sample). |
