...
| contract detection | When `detect_contract_type` flags a JD as contract or freelance, the `match` report appends a **Contract checklist**: duration + extension terms, conversion-to-FTE clause, corp-to-corp vs W-2, benefits/PTO, IP ownership, rate-vs-FTE parity, notice period, exclusivity/non-compete. |
| `contract-compare` | Compare a contract rate (`--rate "$120/hr"`) against an FTE offer (`--fte 200000`): prints an annualized side-by-side table plus the assumptions behind it. Options: `--fte-benefits` ($/yr) and `--hours` (billable hrs/yr, default 2080). |
| freelance sources | `jobs` also covers freelance-heavy boards (`remotive`, `weworkremotely`, `workingnomads`). Contract postings are auto-detected, get the `match` checklist, and carry a `[contract: ...]` note suffix in the tracker. |
