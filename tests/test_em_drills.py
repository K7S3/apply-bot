...
def _clean():
    """Redirect candid.config.DATA_DIR to a fresh temp dir (repo convention).

    config.DATA_DIR is frozen at first import, so the CANDID_DATA_DIR env var
    set above is a no-op when the full suite imports other test modules first.
    Monkeypatching per test keeps these tests isolated and repeatable.
    """
    from candid import config as C
    tmp = Path(tempfile.mkdtemp(prefix="candid-test-em-"))
    C.DATA_DIR = tmp
    return tmp
